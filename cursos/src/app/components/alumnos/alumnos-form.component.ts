...
export class AlumnosFormComponent extends CommonFormComponent<Alumno, AlumnoService> implements OnInit {
  ...
  public crear(): void {
    if (!this.fotoSeleccionada) {
      ...
      .subscribe(alumno =>{
        console.log(alumno);
        Swal.fire('Nuevo:', `Alumno ${this.nombreModel} ${alumno.nombre} creado con éxito`, 'success')
  ...
  public editar(): void {
    if (!this.fotoSeleccionada) {
      ...
      .subscribe(alumno =>{
        console.log(alumno);
        Swal.fire('Modificado:', `Alumno ${this.nombreModel} ${alumno.nombre} actualizado con éxito`, 'success')
        this.router.navigate([this.redirect]);
      }, err => {
        if (err.status === 400) {
          this.error = err.error;
